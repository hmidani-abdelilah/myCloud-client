...
void FileManager::responseBasic(QNetworkReply *reply) {
    if (reply->error() != QNetworkReply::NoError)
        throw HttpError(reply);
}
...
void FileManager::responseFile(QNetworkReply *reply) {
    if (reply->property("verb") == "post") {
        ...
        if (reply->property("replaceFile").toString() == "active")
        ...
        else if (reply->property("deleteFileCanceled").toString() == "active")
            responseBasic(reply);
            ...
            responseDeleteFileOnServer(reply);
    }
}
...
void FileManager::deleteFileOnServer(QString path) {
    QFileInfo fileInfo(path);
    RouteParams prms;

    prms.addQueryItem("pathServer", (fileInfo.path() == "." ? "" : fileInfo.path()));
    prms.addQueryItem("name", fileInfo.fileName());
    _fileRequest->request(FileRequest::DELETE, FileRequest::DefaultFile, prms);
}

void FileManager::responseDeleteFileOnServer(QNetworkReply *reply) {
    if (reply->error() != QNetworkReply::NoError)
        throw HttpError(reply);
    JsonManager json(reply);
    QMap<QString, QVariant> file = json.getJson();
...
/* -------------------------------------------------------------------- */
/* file already exist - replace file action (UPLOAD)
/* -------------------------------------------------------------------- */
...
void FileManager::responseRename(QNetworkReply *reply) {
    ...
    emit fileDeleted(statsOldfFile);
...
void FileManager::statusFileChanged(qint64 id) {
    ...
    case InfoElement::Status::DELETE: {
        ...
        if (file->type() == InfoElement::UPLOAD && file->getSizeTransfering() < file->sizeServer()) {
            ...
            reply->setProperty("deleteFileCanceled", "active");
        }
